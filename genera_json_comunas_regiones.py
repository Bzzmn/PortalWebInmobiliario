...
def generar_json(data):
    ...
    # Iterar sobre cada región en la data
    for i, region in enumerate(data["regiones"], start=1):
        # Agregar la región al resultado
        resultado.append({
            "model": "webPages.Region",
            "pk": i,
            "fields": {
        ...
        # Iterar sobre cada comuna en la región
        for comuna in region["comunas"]:
            # Obtener el índice de la región actual
            region_pk = i
            # Agregar la comuna al resultado
            resultado.append({
                "model": "webPages.Comuna",
                "fields": {
                    "nombre": comuna,
                    "region": region_pk
                }
...
resultado_json = generar_json(data)

# Escribir el resultado en un archivo
with open("resultado.json", "w", encoding="utf-8") as file:
    json.dump(resultado_json, file, indent=4, ensure_ascii=False)
